...
public class ExcelAssetScriptMenu
{
    ...
    const string FieldTemplete = "\tpublic List<#ENTITYFIELDS#> #FIELDNAME# = new(); // Replace 'EntityType' to an actual type that is serializable.";

    [MenuItem("Assets/Create/ExcelAssetScript", false, priority = 4)]
    static void CreateScript()
    {
        //scriptableobject脚本位置
        string savePath = Application.dataPath + "/Scripts/Datas";
        if (savePath == "") return;
        ...
        string path = Path.ChangeExtension(Path.Combine(savePath, "XLSXLOADER_" + excelName), "cs");
